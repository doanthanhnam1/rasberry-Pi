...
int mpu;
unsigned char phai[8]={0x08,0x0c,0xfe,0xff,0xff,0xfe,0x0c,0x08};
unsigned char trai[8]={0x10,0x30,0x7f,0xff,0xff,0x7f,0x30,0x10};
unsigned char len[8]={0x18,0x3c,0x7e,0xff,0x3c,0x3c,0x3c,0x3c};
unsigned char xuong[8]={0x3c,0x3c,0x3c,0x3c,0xff,0x7e,0x3c,0x18};
...
int main(void){
    ...
    float roll = atan2(Ay, sqrt(pow(Ax, 2)+pow(Az, 2)))*180/M_PI;
        if(roll < 0){
            for(int i=0;i<=7;i++)
            sendData(i+1,trai[7-i]);
        }
	else {
	    for(int i=0;i<=7;i++) 
            sendData(i+1,phai[7-i]);
